    ...
    return onAuthStateChanged(firebaseAuth, (account) => {
      void (async () => {
        ...
        try {
          ...
          if (isAuthorizedStaffDocument(data)) {
...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  const [customerRequests, setCustomerRequests] = useState<
    CustomerOrderRequest[]
  >([]);
  const [toppingAvailability, setAvailability] = useState<ToppingAvailability>(
  ...
  useEffect(() => {
    if (!db) localStorage.setItem(PRODUCTS_KEY, JSON.stringify(products));
  }, [products]);
  useEffect(() => {
    if (!db) localStorage.setItem(ORDERS_KEY, JSON.stringify(orders));
  }, [orders]);
  useEffect(() => {
    if (!db)
      localStorage.setItem(
        TOPPING_AVAILABILITY_KEY,
  ...
  const submitOrder = useCallback(
    async (draft: OrderDraft) => {
      if (!draft.items.length)
        throw new Error("ตะกร้าว่าง ไม่สามารถส่งออเดอร์ได้");
      ...
      if (db) {
        ...
        return runTransaction(firestore, async (transaction) => {
          ...
          const order = toFirestoreData(
            createOrder(
              preparedDraft,
              `${date.replaceAll("-", "")}-${padded}`,
              `Q${padded}`,
              user?.uid,
            ),
          );
  ...
  const replaceOrder = async (id: string, draft: OrderDraft) => {
    ...
    if (!current || current.status !== "pending")
      throw new Error("แก้ไขได้เฉพาะออเดอร์ที่รอจัดเตรียม");
    const paymentError = validatePaymentMethod(
      draft.channel,
      draft.paymentMethod,
    );
    ...
    if (db) await updateDoc(doc(db, "orders", id), patch);
      ...
      setOrders((rows) =>
        rows.map((order) => (order.id === id ? { ...order, ...patch } : order)),
      );
  };
  ...
  const setOrderStatus = async (id: string, status: ShopOrder["status"]) => {
    ...
    if (db) {
      const firebasePatch =
        status === "pending"
          ? { ...patch, completedAt: deleteField(), cancelledAt: deleteField() }
          : patch;
  ...
  const dismissCustomerRequest = (id: string) =>
...
export function CartProvider({ children }: { children: ReactNode }) {
  const [items, setItems] = useState<CartItem[]>(() =>
    firebaseReady ? [] : readLocal("gym-cart-v1", []),
  );
    ...
    setItems((rows) => {
      const exact = rows.find(
        (entry) =>
          ...
          normalizeToppingPackaging(entry.toppingPackaging) ===
            normalizeToppingPackaging(item.toppingPackaging) &&
          JSON.stringify(entry.selectedOptionIds) ===
            JSON.stringify(item.selectedOptionIds),
      );
      return exact
        ? rows.map((entry) =>
            entry.id === exact.id
              ? applyCartItemUpdate(entry, {
  ...
  const update = (id: string, patch: Partial<CartItem>) =>
    setItems((rows) =>
      rows.map((item) =>
        item.id === id ? applyCartItemUpdate(item, patch) : item,
      ),
    );
