...
export const toppingPackagingLabels: Record<ToppingPackaging, string> = {
  included: "ใส่ท็อปปิ้งเลย",
  separated: "แยกท็อปปิ้ง",
};
...
export function toppingPackagingLabel(
  item: Pick<CartItem, "toppingPackaging" | "toppingPackagingLabel">,
): string {
  return (
    item.toppingPackagingLabel ??
    toppingPackagingLabels[normalizeToppingPackaging(item.toppingPackaging)]
  );
}
...
export function separatedPackagingAvailabilityError(
  product: Product,
  availability: ToppingAvailability = {},
): string | null {
  if (availability[separatedPackagingAvailabilityId] === false)
    return "แยกท็อปปิ้งหมดชั่วคราว";
  if (!productSupportsSeparatedPackaging(product))
    return "สินค้านี้ไม่รองรับแยกท็อปปิ้ง";
  return null;
}

export function packagingSurchargePerUnit(
  channel: OrderChannel,
  packaging: ToppingPackaging,
): number {
  return packaging === "separated" && getChannelGroup(channel) === "platform"
    ? 5
    : 0;
}
...
export function orderPaymentMethods(order: ShopOrder): PaymentMethod[] {
  const lineMethods = order.items.flatMap((item) =>
    item.paymentMethod ? [item.paymentMethod] : [],
  );
  const methods = lineMethods.length
    ? lineMethods
...
export function paymentMethodsForChannel(
  ...
  return getChannelGroup(channel) === "platform"
    ? ["Platform"]
    : ["สด", "โอน", "โครงการ"];
}
...
export function validatePaymentMethod(
  ...
  return paymentMethodsForChannel(channel).includes(paymentMethod)
    ? null
    : "วิธีชำระเงินไม่ตรงกับช่องทางการขาย";
}
...
export function getChannelGroup(channel: OrderChannel): ChannelGroup {
  return channel === "Lineman" || channel === "Grab"
    ? "platform"
    : "storefront";
}
...
export function getProductPrice(
  ...
  if (channel === "หน้าร้าน")
    return product.channelPrices?.[channel] ?? product.price;
  if (channel === "Openchat")
    return (
      product.channelPrices?.Openchat ??
      product.channelPrices?.["หน้าร้าน"] ??
      product.price
    );
  if (channel === "Grab")
    return (
      product.channelPrices?.Grab ??
      product.channelPrices?.Lineman ??
      product.price
    );
...
export function calculatePriceBreakdown(
  ...
  selectedIds.forEach((id, index) => {
    ...
    if (!topping) return;
    const premium = product.premiumToppingIds
      ? product.premiumToppingIds.includes(id)
      : topping.premium;
    if (index < product.includedToppings)
      premiumIncludedSurcharge += premium ? rules.premiumIncludedSurcharge : 0;
    else if (rules.allowedExtraToppingIds.includes(id))
...
export function validateSelection(
  ...
  if (product.optionMode === "granola" && selectedIds.length !== 1)
    return "กรุณาเลือกรสกราโนล่า 1 รส";
  if (
    product.optionMode === "toppings" &&
    selectedIds.length < product.includedToppings
  )
    return `กรุณาเลือกท็อปปิ้งอย่างน้อย ${product.includedToppings} อย่าง`;
  if (
    selectedIds.some((id) => !isSelectionAvailable(product, id, availability))
  )
  ...
  if (product.optionMode === "toppings") {
    ...
    if (
      ...
      return "ช่องทางนี้ไม่อนุญาตให้เลือกท็อปปิ้งซ้ำ";
    const unsupported = selectedIds
      .slice(product.includedToppings)
      .find((id) => !rules.allowedExtraToppingIds.includes(id));
    ...
    isSelectionAvailable(product, id, availability)
      ...
      : [
          item.selectedOptions[index] ??
            available.find((entry) => entry.id === id)?.name ??
            id,
        ],
  );
  const validationError = unavailable.length
    ? `หมด: ${unavailable.join(", ")}`
    : (validateSelection(
        product,
        item.selectedOptionIds,
        channel,
...
export function applyCartItemUpdate(
  item: CartItem,
  ...
  const packagingSurcharge =
    patch.packagingSurchargePerUnit ?? item.packagingSurchargePerUnit ?? 0;
  ...
  if (
    Object.prototype.hasOwnProperty.call(patch, "validationError") &&
  ...
  return items.map((item) => {
    ...
    return product
      ...
      : {
          ...item,
          selectedChannel: channel,
          validationError: "ไม่พบสินค้านี้ในเมนูปัจจุบัน",
        };
  });
}
...
export function prepareOrderItems(
  ...
  if (invalid)
    throw new Error(`${invalid.productName}: ${invalid.validationError}`);
  return priced.map((item) => {
    const prepared = {
      ...
      packagingSurchargeTotal:
        (item.packagingSurchargePerUnit ?? 0) * item.quantity,
    };
...
export function nextLocalQueue(
  orders: ShopOrder[],
  date = businessDate(),
): { sequence: number; id: string; queue: string } {
  const sequence =
    orders
      .filter((order) => order.businessDate === date)
      .reduce(
        (max, order) =>
          Math.max(max, Number(order.queueNumber.replace(/\D/g, "")) || 0),
        0,
      ) + 1;
  const padded = String(sequence).padStart(3, "0");
  return {
    sequence,
    id: `${date.replaceAll("-", "")}-${padded}`,
    queue: `Q${padded}`,
  };
}
...
export function createOrder(
  ...
  const items = draft.items.map((item) => {
    const packaging = normalizeToppingPackaging(item.toppingPackaging);
    const packagingSurcharge =
      item.packagingSurchargePerUnit ??
      packagingSurchargePerUnit(draft.channel, packaging);
    const snapshot = {
      ...item,
      selectedChannel: item.selectedChannel ?? draft.channel,
      toppingPackaging: packaging,
      toppingPackagingLabel:
        item.toppingPackagingLabel ?? toppingPackagingLabels[packaging],
      packagingSurchargePerUnit: packagingSurcharge,
      packagingSurchargeTotal: packagingSurcharge * item.quantity,
      lineTotal: item.unitPrice * item.quantity,
    };
  ...
  const order: ShopOrder = {
    id,
    queueNumber,
    businessDate: businessDate(new Date(now)),
    customerName: draft.customerName.trim() || "ลูกค้าทั่วไป",
