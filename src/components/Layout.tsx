...
const nav = [
  ...
  { to: staffOrderPath, label: "สั่งสินค้า", icon: PlusCircle },
  { to: "/queue", label: "คิว", icon: ClipboardList },
...
export default function Layout({ children }: { children: ReactNode }) {
  ...
  const { items, channel } = useCart();
  const { orders, customerRequests } = useData();
    ...
    <div className="app-shell">
      <aside className="sidebar">
        <NavLink className="brand" to="/">
          <span className="brand-mark">G&amp;M</span>
          <span>
            <strong>Greek &amp; More</strong>
        ...
        <nav>
          {nav.map(({ to, label, icon: Icon }) => (
            <NavLink key={to} to={to} end={to === "/"}>
              ...
              <PendingBadge count={badgeFor(to)} />
            </NavLink>
          ))}
        </nav>
        ...
        <header className="topbar">
          <div>
            ...
            <span className="demo-pill">
              {runtimeConfig.isCustomerQrUat || isDemo
                ? "โหมดทดลอง"
                : "ระบบจริง"}
            </span>
            {channel && (
              <span className="channel-pill">{channelLabels[channel]}</span>
            )}
          </div>
          <NavLink className="cart-chip" to="/cart">
            <ShoppingBasket /> ตะกร้า {quantity > 0 && <b>{quantity}</b>}
          </NavLink>
        </header>
        <main>{children}</main>
