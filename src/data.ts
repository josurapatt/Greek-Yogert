...
const premiumToppings = toppings
  .filter((topping) => topping.premium)
...
export const platformExtraToppingIds = [
  "granola-banana",
  "granola-berry",
  "granola-chocolate",
  "granola-honey",
...
const channelConfig = (storefront: number, platform: number) => ({
  channelPrices: {
    ...
    Lineman: platform,
    Grab: platform,
  } satisfies Record<OrderChannel, number>,
...
export const defaultProducts: Product[] = [
  ...
  },
  {
    ...
    price: 59,
    emoji: "🥣",
    description: ["กรีกโยเกิร์ต 1 สกู๊ป (65 กรัม)", "คอร์นเฟลกส์", "น้ำผึ้ง"],
    optionMode: "none",
    includedToppings: 0,
    granolaOptions: [],
    availableToppingIds: [],
  },
  {
    ...
    description: [
      ...
      "คอร์นเฟลกส์",
      "น้ำผึ้ง",
      "เลือกท็อปปิ้ง 3 อย่าง",
    ],
    optionMode: "toppings",
    includedToppings: 3,
    granolaOptions: [],
    availableToppingIds: allToppings,
    premiumToppingIds: premiumToppings,
  },
  {
    ...common,
    ...channelConfig(99, 119),
    id: "size-m",
    name: "Size M",
    price: 99,
    emoji: "🍧",
  ...
  },
  {
    ...
    price: 10,
    emoji: "🥜",
    description: ["กราโนล่าเปล่า 1 ถ้วย", "เลือกรสชาติ 1 รส"],
    optionMode: "granola",
    includedToppings: 0,
    granolaOptions,
    availableToppingIds: [],
...
export function normalizeProduct(product: Product): Product {
  ...
  prices.Lineman =
    ...
    seeded?.channelPrices?.Lineman ??
    basePrice;
  prices.Grab =
    product.channelPrices?.Grab ??
    seeded?.channelPrices?.Grab ??
    prices.Lineman;
  return {
    ...(seeded ?? {}),
    ...product,
    price: prices["หน้าร้าน"],
    channelPrices: prices,
    supportsSeparatedToppingPackaging:
      product.supportsSeparatedToppingPackaging !== false,
    maxSelectedOptions:
      product.optionMode === "none"
        ? 0
        : product.optionMode === "granola"
          ? 1
          : (product.maxSelectedOptions ?? 10),
    channelRules: {
      storefront: {
        ...storefrontRules,
        ...(seeded?.channelRules?.storefront ?? {}),
      ...
      platform: {
        ...platformRules,
        ...(seeded?.channelRules?.platform ?? {}),
        ...(product.channelRules?.platform ?? {}),
      },
    },
  };
}
